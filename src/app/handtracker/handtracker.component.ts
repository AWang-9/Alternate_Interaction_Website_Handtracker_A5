  ...
  */
  ...
  startDetection(){
    this.startVideo().then(()=>{
        ...
        this.video.nativeElement.style.height = "110px"
    ...
    if (this.model != null){
        let predictions = this.model.detect(this.video.nativeElement).then((predictions: any) => {
            ...
            for(let p of predictions){
                ...
                this.all_info = p.label + " at X: " + p.bbox[0] + ", Y: " + p.bbox[1] + " at X: " + p.bbox[2] + ", Y: " + p.bbox[3];
                this.bounding_boxes = p.bbox;
            ...
            if(pointing == 1 && openhands == 1) this.detectedGesture = "One Point One Open";

            if(closedhands == 1 && openhands == 1) this.detectedGesture = "One Closed One Open";

            if (openhands == 0 && closedhands == 0 && pointing == 0 && pinching == 0)
                this.detectedGesture = "None";

            this.onPrediction.emit(new PredictionEvent(this.detectedGesture, this.all_info, this.bounding_boxes))
        }, (err: any) => {
            console.log("ERROR")
            console.log(err)
        });
